...
function renderProduce(produceData) {
  ...
  produceData.forEach((item) => {
      ...
      carbonText.textContent = `Carbon Credits: ${item.produce_carbonCredit}`; // Updated key
    ...
    priceDiscount.textContent = `SAVE ${item.discount * 100} %`;
      ...
      cardBody.append(title, farmText, priceText, quantityText, carbonText, priceDiscount);
...
//       .then(data => {
//           const filteredProduce = category === "All" ? data.produce : data.produce.filter(item => item.category === category);
